...
def figure(amount_of_angles, color, point=sd.get_point(200,150), angle=0, length=200):
    ...
    for x in range(0, 360 - step, step):
        ...
        v1.draw(color=color)
        sd.circle(center_position=v1.end_point, color=color, radius=30)           # круги на углах
        point = v1.end_point # переход на следующюю точку
    else:
        sd.line(start_point=point, color=color, end_point=last_point_skleika, width=1)
        sd.circle(center_position=last_point_skleika, color=color, radius=30)       # круг на первой точке
...
while True:                                 # Ввод пользователем номера и проверка
    user_input = input()
...
figure(amount_of_angles, color)       # Вызов функции с обязательными параметрами фигуры и цвета
